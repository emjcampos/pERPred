...
```{r hex sticker, echo = FALSE, out.width = "12%", out.extra='style="float:right; padding:10px"'}
knitr::include_graphics("pERPred.png")
```

The `pERPred` package is a tool for conducting ERP analyses using the Principle ERP Reduction algorithm put forth in (cite our paper) by Campos and Hazlett et al (2019). This package contains the functions necessary to derive the principle ERPs discussed in that publication as well as the tools for performing "pERP-space analysis". Let this vignette serve as a guide to our approach to ERP analysis in `R`. 
...
\begin{equation*}
Y_{i,v,e}(t) = \sum_{c=1}^C k_{c,v,e}\phi^{\star}_c(t) + \sum_{c=1}^C \xi_{c,i,v,e}\phi^{\star}_c(t) + \zeta_{i,v,e}(t).
\end{equation*}
...
By default, the percent of variation is set to 80. Raising this value means more components will be kept in each of the PCA steps and the computation time will rise accordingly. Also, the number of pERPs is chosen based on an $R^2$ value defined as: 
\begin{equation*} 
R_{test}^2 = 1 - \frac{|| Y_j - \Phi \beta_j ||_{\mathcal{F}}^2}{|| Y_j  ||_{\mathcal{F}}^2}
\end{equation*} 
...
Now we can reconstruct each record using their weights and the pERPs. Here is an example using subject 1, electrode 1, task 1.

```{r individual reconstruction}
record <- simulated_data %>% 
  filter(Subject == "1", 
         Task == "1") %>% 
  select(Task, Time, "electrode_1")
...
projected <- data.frame("Projected" = as.matrix(pERPs5) %*% 
  ...
  mutate(Projected = Projected + mean(record$electrode_1))

ggplot() + 
  geom_line(data = record, 
            aes(x = Time, y = electrode_1, color = "Observed")) + 
