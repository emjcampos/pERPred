...
# Principle ERP Reduction (pERP-RED) <img src = 'pERPred.png' align = "right" height = "138.5" />

The `pERPred` package is a tool for conducting ERP analyses using the Principle ERP Reduction algorithm put forth by Campos and Hazlett et al (2020). This package contains the functions necessary to derive the principle ERPs discussed in that publication as well as the tools for performing "pERP-space analysis". Let this vignette serve as a guide to our approach to ERP analysis in `R`. 
...
pERPs <- map(
  3:7, 
  ~ pERPred(
    simulated_data[simulated_data$Subject %in%
                     train_subjects, ],
    num_pERPs = .x,
