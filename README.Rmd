...
```{r hex_sticker, echo = FALSE, out.width = "12%", out.extra='style="float:right; padding:10px"'}
knitr::include_graphics("man/figures/pERPred.png")
```

The `pERPred` package is a tool for conducting ERP analyses using the Principle ERP Reduction algorithm put forth in (cite our paper) by Campos and Hazlett et al (2019). This package contains the functions necessary to derive the principle ERPs discussed in that publication as well as the tools for performing "pERP-space analysis". Let this serve as a guide to our approach to ERP analysis in `R`. 
